# -*- coding: utf-8 -*-
"""
Created on Wed Nov 29 23:49:56 2017
...
dir="C:\\Users\\Ashwini\\.spyder-py3\\resizedImages"
search="C:\\Users\\Ashwini\\.spyder-py3\\resizedImages\\search.png"
...
OPENCV_METHODS = (
	("Correlation", cv2.HISTCMP_CORREL),#cv2.cv.CV_COMP_CORREL
...
for (methodName, method) in OPENCV_METHODS:
    ...
    for (k, hist) in index.items():
        ...
        d = cv2.compareHist(index["search.png"], hist, method)
        results[k] = d
...
for (methodName, method) in SCIPY_METHODS:
	...
	# loop over the index
    for (k, hist) in index.items():
        d = method(index["search.png"], hist)
        results[k] = d
		# compute the distance between the two histograms
		# using the method and update the results dictionary
...
for (k, hist) in index.items():
	...
	d = chi2_distance(index["search.png"], hist)
	results[k] = d
